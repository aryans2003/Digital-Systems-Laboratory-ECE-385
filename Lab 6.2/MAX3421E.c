...
uint8_t writebuffer[2] = {(reg + 2), val};
int ret = XSpi_Transfer(&SpiInstance, writebuffer, NULL, 2);
if(ret != XST_SUCCESS)
{
xil_printf("error");
}
...
{
arr[1] = data[i];
int ret = XSpi_Transfer(&SpiInstance, arr, NULL, 2);
if(ret != 0)
{
xil_printf("error");
}
...
//read_buffer[0] = reg;
write_buffer[1] = 0;
int ret = XSpi_Transfer(&SpiInstance, write_buffer, read_buffer, 2);
if(ret != XST_SUCCESS)
{
xil_printf("error");
}
